...
namespace Game_Caro
{
    partial class GameCaro : Form
    {
        ...
        {
            lbl_About.Text = "Game Caro project with\nC# WinForms\n-- ♦ ♦ ♦ --\nBùi Đình Ngọc \nPhi Anh Tú";
            tm_About.Enabled = true;
        }

        private void GameCaro_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (MessageBox.Show("Bạn có chắc muốn thoát không", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != System.Windows.Forms.DialogResult.Yes)
                e.Cancel = true;
            else
            {
                try
        ...
        private void Board_PlayerClicked(object sender, BtnClickEvent e)
        {
            ...
            if (board.PlayMode == 1)
            {
                ...
                {
                    EndGame();
                    MessageBox.Show("Không có kết nối nào tới máy đối thủ", "Lỗi kết nối", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void ViaLANToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!socket.ConnectServer())
            {
                ...
                pn_GameBoard.Enabled = true;
                socket.CreateServer();
                MessageBox.Show("Bạn đang là Server", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
        ...
        private void SameComToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (board.PlayMode == 1)
            {
                ...
                socket.CloseConnect();
                MessageBox.Show("Đã ngắt kết nối mạng LAN", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            ...
            switch (data.Command)
            {
                ...
                case (int)SocketCommand.QUIT:
                    this.Invoke((MethodInvoker)(() =>
                    {
                        ...
                        MessageBox.Show("Đối thủ đã chạy mất dép", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }));
                    break;
