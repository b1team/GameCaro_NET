...
namespace Game_Caro
{
    class SocketManager
    {
        ...
        public void CreateServer()
        {
            ...
            server.Bind(iep);
            server.Listen(10); // Đợi kết nối client trong 10s nếu ko có thì bỏ

            Thread AcceptClient = new Thread(() => { try { client = server.Accept(); } catch { } });
            AcceptClient.IsBackground = true; // Để khi chương trình tắt ngang thì Thread cũng tự tắt
            AcceptClient.Start();
        }
        #endregion
        ...
        {
            byte[] receivedData = new byte[BUFFER]; // 1 lần nhận tin là cỡ bao nhiêu
